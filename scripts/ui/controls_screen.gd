...
func _show_keyboard_page() -> void:
	...
	_pair_nav_button.text = "JUGADOR 3 y 4  →"
...
func _update_keyboard_pair() -> void:
	...
	if p1 != null:
		p1.visible = _keyboard_pair == 0
	if p2 != null:
		p2.visible = _keyboard_pair == 0
	if p3 != null:
		p3.visible = _keyboard_pair == 1
	if p4 != null:
		p4.visible = _keyboard_pair == 1

	_pair_nav_button.text = "JUGADOR 3 y 4  →" if _keyboard_pair == 0 else "←  JUGADOR 1 y 2"
...
func _set_keyboard_content_visible(is_visible: bool) -> void:
	for node_name in ["Title", "Players"]:
