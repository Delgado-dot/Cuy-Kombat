...
func _update_leader() -> void:
	var best_index := 0
	var best_health := -1.0
	for i in range(mini(_players.size(), _display_health.size())):
		if _display_health[i] > best_health:
			best_health = _display_health[i]
			best_index = i

	for i in range(mini(_players.size(), 4)):
		var label := _get_leader_label(i)
		if label != null:
			label.visible = (i == best_index and _display_health[i] < 0.99)
...
func _update_stars() -> void:
	...
	_p3_stars.text = _stars_text(RoundManager.get_wins(3))
	_p4_stars.text = _stars_text(RoundManager.get_wins(4))
