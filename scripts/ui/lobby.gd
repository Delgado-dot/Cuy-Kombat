...
func _update_player_display(player_num: int) -> void:
	...
	if pivot != null:
		...
		if ResourceLoader.exists(scene_path):
			...
			if packed != null:
				# Single, independent instance per player/pivot (PackedScene.instantiate
				# always returns a fresh Node3D). No duplicate() of an unparented node:
				# the previous model of THIS pivot was already freed above, so P1 and P2
				# never share an instance, a pivot, or a SubViewport.
